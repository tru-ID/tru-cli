...
export default abstract class ChecksCreateCommand extends CommandWithProjectConfig {
  ...
  static flags = {
    ...
    'skip-qrcode-handler': flags.boolean({
      description:
        'Skips using the tru hosted QR code handler with the `check_url`',
      required: false,
      dependsOn: ['workflow'],
    }),
  }
  ...
  async run() {
    ...
    if (response.status !== CheckStatus.ACCEPTED) {
      ...
      if (this.flags.workflow) {
        const oAuth2APIClient = new OAuth2APIClient(
          apiConfiguration,
          this.logger,
        )
        ...
        if (!this.flags['skip-qrcode-handler']) {
          const handlerUrl: string =
            this.globalConfig?.qrCodeUrlHandlerOverride ??
        ...
        this.log(
          chalk.white.bgRed.visible(
            `Please ensure the mobile phone with the phone number ${this.args.phone_number} is disconnected from WiFi and is using your mobile data connection.`,
          ),
        )
        this.log('')
        ...
        try {
          const checkResponse = await this.waitForFinalCheckState(
            checkApiClient,
            response,
          )
