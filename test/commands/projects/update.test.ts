...
describe('Command: projects:update', () => {
  ...
  test
    ...
    .it(
      'should load local project configuration if no project ID arg is provided',
      () => {
        expect(existsSyncStub).to.have.been.calledWith(
          workingDirectoryProjectConfigPath,
        )
      },
    )
  ...
  test
    ...
    .it(
      'YYY should use project ID from local project configuration if no project ID arg is provided',
      () => {
  ...
  test
    ...
    .exit(1)
    .it(
      'shows error message if local config is loaded but no update flags are provided',
      () => {
  ...
  test
    ...
    .command(['projects:update', 'f0f5fb8e-db1c-4e75-bae8-cvxcvxcv'])
    .exit(1)
    .it('shows error message if no update flags are provided', () => {
      expect(consoleLoggerErrorStub).to.have.been.calledWith(
        'At least one flag must be supplied to indicate the update to be applied to the Project',
      )
    })
  ...
  test
    ...
    .exit(1)
    .it(
      'should show an error message if an empty string is supplied for the --phonecheck-callback flag',
      () => {
        expect(consoleLoggerErrorStub).to.have.been.calledWith(
          '"phonecheck-callback" must be a valid URL',
        )
  ...
  test
    ...
    .it(
      'should call the API client `update` is called to update existing project',
      () => {
        expect(projectsApiUpdateStub).to.have.been.calledWith(
          createProjectAPIResponse.project_id,
          {
  ...
  test
    ...
    .it('should inform the user that the update was successful', () => {
      expect(consoleLoggerInfoStub).to.have.been.calledWith(
        '✅ Project updated',
      )
    })
  ...
  test
    ...
    .it('should call the API client with the callback_url removed', () => {
      expect(projectsApiUpdateStub).to.have.been.calledWith(
        createProjectAPIResponse.project_id,
        {
          configuration: {
  ...
  test
    ...
    .it('should call the API client with mode of sandbox', () => {
      expect(projectsApiUpdateStub).to.have.been.calledWith(
        createProjectAPIResponse.project_id,
        {
          mode: 'sandbox',
  ...
  test
    ...
    .it('should call the API client with mode of live', () => {
      expect(projectsApiUpdateStub).to.have.been.calledWith(
        createProjectAPIResponse.project_id,
        {
          mode: 'live',
