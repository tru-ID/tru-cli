...
describe('hooks', () => {
  ...
  .exit(1)
  .it('should instruct the user to go to developer.tru.id/console when no config file is present and a command is run', ctx => {
    expect(ctx.stdout).to.contain('Welcome to the tru.ID CLI!')
    expect(ctx.stdout).to.contain('developer.tru.id/console')
  })
  ...
  .command(['setup:credentials', expectedUserConfig.defaultWorkspaceClientId, expectedUserConfig.defaultWorkspaceClientSecret, expectedUserConfig.defaultWorkspaceDataResidency])
  .it('should NOT instruct the user to setup the CLI if the user is running the `setup:credentials` command', () => {
    expect(outputJsonStub).to.have.been.calledWith(sinon.default.match.any, expectedUserConfig, sinon.default.match.any)
  })
})
